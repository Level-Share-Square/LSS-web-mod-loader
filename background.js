...
let replaceRoot = "https://levelsharesquare.com/html5/supermarioconstruct/images";
...
const reloadModRules = async (mods) => {
	...
	chrome.storage.local.get(null, (result) => {
		// load the mods
		const allMods = Object.entries(result).map(([name, mod]) => ({ name, ...mod })) || [];
		...
		console.log(imageNames)//images.map(image => image[0][0]))
		for (var img of images) {
			var mappings = {}
			mappings[`${replaceRoot}/${img.name}`] = `${img[0]}`
			console.log(`${replaceRoot}/${img.name} \n${img[0]}`)
