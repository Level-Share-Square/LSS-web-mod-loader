...
const createRemoveButton = (mod, listItem) => {
  ...
  removeButton.addEventListener("click", async () => {
    // confirmation
    const proceed = await confirm(chrome.i18n.getMessage("remove_mod_confirm"));
    if (!proceed) return;
...
const createToggleButton = (mod) => {
  ...
  toggleModButton.title = mod.enabled
    ? chrome.i18n.getMessage("click_to_disable")
    : chrome.i18n.getMessage("click_to_enable");
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response.type === CONSTANTS.MOD_TOGGLED) {
            // update message
            toggleModButton.title == chrome.i18n.getMessage("mod_disabled")
              ? chrome.i18n.getMessage("mod_enabled")
              : chrome.i18n.getMessage("mod_disabled");
            // update class
