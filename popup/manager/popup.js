...
document.getElementById("uploadModFolder").addEventListener("change", function () {
...
document.getElementById("replacementForm").addEventListener("submit", async (event) => {
	...
	if (!loaderFile) {
		alert("This isn't a valid modpack folder, make sure it has a loader.json file at the root of the folder!");
		return;
	}
	...
	reader.addEventListener("load", async () => {
		...
		for (const image of images) {
			...
			const imagePath = image.webkitRelativePath.replace(normalizedImageRoot + "/", "");
