  ...
  if (message.type === CONSTANTS.GAME_DETECTED) {
    extension.storage.session.get("surpressPopup", (data) => {
      ...
      extension.windows.create({
        ...
        type: "popup",
        width: 500,
        height: Math.round(screen.height) - 40,
        top: 50,
        left: 0,
        focused: true,
      });
...
const createBlobUrl = async (base64) => {
 ...
 */
const handleDynamicRuleUpdate = async (newImages, resolve) => {
  // Convert base64 images to Blob URLs (Firefox fix)
  const imageMappings = await Promise.all(
    newImages.map(async ({ data, path }, index) => {
      const blobUrl = await createBlobUrl(data);
      return { id: index + 1, path, blobUrl };
    })
  );
  // define the rules
  const newRules = await imageMappings.map(({ id, path, blobUrl }) => ({
    id,
    priority: 1,
    action: {
      type: "redirect",
      redirect: { url: blobUrl },
    },
    condition: {
